...
type
  TForm1 = class(TForm)
    ...
    procedure LayoutNesnePainting(Sender: TObject; Canvas: TCanvas;
...
begin
  with LayoutNesne do
  begin
    if Position.X <> (LayoutWorld.Width - Width) * 0.1 then
      Position.X := (LayoutWorld.Width - Width) * 0.1; // Ortayý Getir
...
begin
  ...
  if LayoutWorld.Tag < btnBasla.Tag then
  begin
    LayoutWorld.Tag := btnBasla.Tag;
  end;

end;

procedure TForm1.LayoutNesnePainting(Sender: TObject; Canvas: TCanvas;
  const [Ref] ARect: TRectF);
begin
  Canvas.ClearRect(ARect, TAlphaColors.Yellow);
  // nesnenin kapladýðý yeri sarýya boya
end;
...
begin
  ...
  H.Y := 3; // yukarý ve aþaðý standart yakýnlýk
